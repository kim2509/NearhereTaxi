...
/**
 * 사용자정보 저장 요청 ({@link UserManagement#requestUpdateProfile(UpdateProfileResponseCallback, java.util.HashMap)}) 호출할 때 넘겨주고 콜백을 받는다.
 * @author MJ
 */
public abstract class UpdateProfileResponseCallback extends UserResponseCallback {
    ...
    protected void onSuccessUser(final User user) {
        if(user == null || user.getId() <= 0)
            onError("UpdateProfileResponseCallback : onSuccessUser is called but the result user is null.", new APIErrorResult("the result of updateProfile request is null."));
        else
