...
/*
 * revised com.ning.http.client.providers.jdk.JDKAsyncHttpProvider by deleting unnecessary code.
 *
...
public class SimpleAsyncHttpProvider implements AsyncHttpProvider {
    //private final static Logger logger = LoggerFactory.getLogger(SimpleAsyncHttpProvider.class);
...
//        }
//
//        SimpleFuture f = delegate == null ? new SimpleFuture<T>(handler, requestTimeout, urlConnection) : delegate;
        SimpleFuture f = new SimpleFuture<T>(handler, requestTimeout, urlConnection);
...
//            request.getURI().toURL().openConnection(proxy == null ? Proxy.NO_PROXY : proxy);
        HttpURLConnection urlConnection = (HttpURLConnection)
            request.getURI().toURL().openConnection(Proxy.NO_PROXY);
...
//                }
                ...
                ResponseStatus status = new ResponseStatus(uri, urlConnection, SimpleAsyncHttpProvider.this);
                ...
                if (redirectEnabled && (statusCode == 302 || statusCode == 301)) {
                    ...
                    if (currentRedirectCount++ < config.getMaxRedirects()) {
                        ...
                        if (!newUrl.equals(uri.toString())) {
                            ...
                            logger.d("Redirecting to " + newUrl);
                ...
                if (statusCode == 401 && !isAuth.getAndSet(true) && realm != null) {
                    ...
                    logger.d("Sending authentication to " + request.getUrl());
                ...
                if (state == AsyncHandler.STATE.CONTINUE) {
                    state = asyncHandler.onHeadersReceived(new ResponseHeaders(uri, urlConnection, SimpleAsyncHttpProvider.this));
                }
                ...
                if (state == AsyncHandler.STATE.CONTINUE) {
                    ...
                    if (byteToRead > 0) {
                        ...
                        while (leftBytes > -1) {
                            ...
                            leftBytes -= read;
                            asyncHandler.onBodyPartReceived(new ResponseBodyPart(uri, b, SimpleAsyncHttpProvider.this, leftBytes > -1));
                        }
                    }

                    if (request.getMethod().equalsIgnoreCase("HEAD")) {
                        asyncHandler.onBodyPartReceived(new ResponseBodyPart(uri, "".getBytes(), SimpleAsyncHttpProvider.this, true));
                    }
                }
...
//            }
            ...
            if (request.getHeaders().getFirstValue("User-Agent") != null) {
                ...
                urlConnection.setRequestProperty("User-Agent", AsyncHttpProviderUtils.constructUserAgent(SimpleAsyncHttpProvider.class));
            }
